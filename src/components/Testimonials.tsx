  ...
  const renderStars = () => {
    return (
      <div className="flex mb-4">
        {[...Array(5)].map((_, i) => (
          <Star key={i} className="h-5 w-5 text-brand-warning fill-brand-warning" />
        ))}
      </div>
    );
  };
        ...
        <div className="mb-10">
          <div className="flex justify-between items-center mb-8">
            ...
            <div className="flex gap-2">
              <button 
                onClick={handlePrev} 
                className="p-2 rounded-full bg-brand-primary/10 hover:bg-brand-primary/20 transition-colors"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {visibleTestimonials.map((testimonial) => (
              <div 
                key={testimonial.id} 
                className="bg-gradient-to-b from-brand-primary/10 to-transparent p-6 rounded-xl backdrop-blur-sm border border-brand-primary/20 card-hover"
              >
                {renderStars()}
